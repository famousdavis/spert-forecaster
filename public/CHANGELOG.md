...
- `MergeImportDialog` component removed; `merge-import.ts` deleted.
...
- Import state machine extracted to `useImportState()` hook.
- Merge computation now fully atomic inside Zustand's `set()` updater, with
  conflict re-detection at write time. Prevents both concurrent-add and
  concurrent-delete drift across tabs.
- 866 tests / 40 files passing.
