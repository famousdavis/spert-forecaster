...
### Changed
- **The note now says eight, and says why.** The note now says eight, and says why. It also warns against “correcting” the references to nine that remain in the same note: those describe the July 2026 audit, when there really were nine, and changing them would make them false.
...
The SPERT® Suite business — the spertsuite.com website, the hosting it runs on, and the SPERT®, Statistical PERT® and Estimation Made Easy® trademarks — transferred to famousdavis, LLC on September 12, 2026. The licence's trademark clause still named the previous owner while the newly published Terms of Service named the company, so **two published legal documents disagreed about who owns the marks**.

### Changed
- **The trademark clause now names famousdavis, LLC.** Fifteen lines, all inside that one clause. The file is still 756 lines and the clause is still what it was: a declination to grant trademark rights, permitted by Section 7(e) of the GPL.

### Notes
- **⚠️ Copyright in the software did not transfer, and the licence still says so.** It is held by William W. Davis, MSPM, PMP personally and licensed to the company. The clauses requiring his name to be kept as the author, and withholding his name from promotional use, are unchanged — as are the copyright notices in every source file, the footer, and the About tab's credit. Two different names now appear in the licence and both are correct: the company operates the service, the individual owns the code.
...
The post-release check added in v0.43.3 went out to the nine projects in the suite in two slightly different forms. Partway through that rollout one project's code-style tool objected to how a line in it was written, so the line was rewritten; the projects that had already received the file kept the first version.

The two behave identically — the corrected one computes a value before using it rather than nesting one expression inside another. But these scripts are deliberately the same file in every project, so that none can quietly drift onto its own version of the release rules, and **two forms in circulation is exactly the drift that arrangement exists to prevent**.

### Changed
- **Every project now carries the corrected form of the post-release check.** This one included; the pre-release gate script is unchanged.
...
Releasing this app includes bringing the local copy of the project back into line with the copy on the server once a release has landed. Merging advances the server; it does not touch the local copy. If the local one is left behind, every report still reads as though the release arrived everywhere, and the next release is then built on the wrong starting point. That step existed only as a line in a written checklist.

Two mechanisms that look like they should have covered it could not. The pre-release check runs before a release is merged, so the condition it would be checking does not exist yet. The automated build cannot check it either, because it is a fact about the machine doing the release rather than about the project, and a fresh automated copy has no view of anyone's working copy. **The gap was invisible rather than merely unaddressed**, which is why it survived from the day the gate was introduced.

### Added
- **A pre-release check that the release is not being cut on a stale base.** The gate now refuses to proceed if the local copy is behind the server. No other gate check can see that, because they all read the working tree, which looks entirely correct. Being ahead is normal and is not reported.
- **A post-release command that verifies the local copy actually advanced.** It compares three sources — the local copy, the local record of the server, and the server's own answer — and reports which one disagrees. Three rather than two, because the first two can be stale together and agree with each other while both are wrong.
...
- **A fetch failure fails the pre-release check rather than skipping it.** The comparison reads a remote-tracking ref, so a swallowed error would compare a stale ref with itself, agree, and read as a pass.
- **The release script stays byte-identical across the suite.** It is deliberately the same file in every SPERT® repo so no one app can drift onto its own rules; this repo now carries the same version as the others.
...
The Forecaster can import a file exported by SPERT® Story Map. For that to work, Story Map has to know what this app will accept — how long a name may be, how many milestones a project may carry, what counts as a real calendar date — so that it can refuse to hand you a file the Forecaster would only reject. It knows those rules because they are written down a second time, by hand, inside Story Map.

Two copies of one set of rules drift apart, and the awkward part is which side notices. Story Map cannot see this app's source, so nothing over there can tell when a rule here is tightened. This release puts the alarm on the side where the change actually happens: alter one of these rules in the Forecaster and a check in the Forecaster fails, in the same piece of work that altered it, rather than surfacing weeks later as a Story Map export that will not import.

Doing this turned up something closer to home. The limit of ten milestones per project exists in three separate places — the screen that stops you adding an eleventh, the importer that rejects a file carrying eleven, and Story Map's copy — and none of the three was connected to the other two. Raising the limit on the screen would have let the app build projects its own importer refuses. That is now held together by a check.

### Added
- **A file exported by Story Map is now run through this app's real import machinery on every test run.** Not a stand-in and not a re-description of the rules — the same code that reads the file when you press Import. The file itself is the exact one Story Map keeps, copied here byte for byte and pinned by a fingerprint recorded in both projects, so a quietly edited local copy fails rather than passing against itself.
- **The check that the file is recognised as coming from Story Map, specifically.** This is the one that matters and the one easiest to get wrong: a Story Map file can be accepted as valid while still being treated as an ordinary Forecaster file, which is what happened for the whole life of the feature until Story Map began labelling its exports. Treated that way, importing offers to replace your entire workspace instead of offering the per-project merge choices.
- **Every limit is now checked from both sides.** A name of exactly two hundred characters must be accepted and one of two hundred and one must be refused; ten milestones accepted, eleven refused; and so on for each numeric limit, at its ceiling and at its floor of zero. Checking only one side passes just as happily when the limit itself is set wrong, which is a mistake this work has now made three times.
- **A date check built on a real leap year rather than an obviously impossible date.** The 29th of February 2028 must be accepted and the 29th of February 2027 refused. An impossible date such as the 45th of the 13th month is rejected by the first and cruder of the two tests the app applies, so it never reaches the second; a non-leap 29th of February passes the first test cleanly and is silently shifted to the 1st of March, so only the second test catches it. Just one of the two pairs proves the app is consulting a calendar rather than matching a pattern.
- **A count of the app's rejection rules, worked out from the source rather than written down.** Story Map keeps a list describing all thirty-three ways this app can refuse a file. Over there the total is a figure someone typed. Here it is counted from the file itself, so adding a thirty-fourth rejection without describing it fails immediately.
- **A check that every described rejection still produces the wording recorded for it.** Each is matched on its message rather than on a line number, because a line number pointing into another project decays the first time this file is edited.

### Notes
- **What this closes, and what it does not.** It closes one direction: this app tightening a rule while Story Map's copy stays behind. The opposite direction — a change in Story Map newly running into a rejection here — was closed separately, on the Story Map side. A third gap remains open and is written into the files: if Story Map edits its own description of a rule and this app does not, every check here still passes, because no check here can read the other project. Re-copying is a manual step at each Story Map release.
- **The limits that are checked here are still typed by hand in two places.** Generating them instead was considered and rejected: it would relocate the duplication rather than remove it, and a generated copy needs its own staleness check, which is the problem it was meant to solve. Removing the second copy altogether needs a shared package, which the Suite has deliberately never created. What has changed is not that the copy is gone — it is that the copy going stale is now noticed here first.
- **Counting rejection rules by reading the text of the source is a blunt instrument, and the file says so.** It cannot see a rejection raised from a helper rather than written in place, and it will report a harmless rearrangement as a failure. It is kept because it needs nothing outside this project — no network, no second copy of Story Map on the machine, no pointer that rots. The stronger alternative is recorded alongside it so the next person does not have to work it out again.
- **Every check was run against a deliberately broken version of itself before being kept.** Fourteen faults were introduced one at a time — a single byte changed in the copied file, a described rejection deleted, a thirty-fourth rejection added, each side of a limit inverted, the Story Map label ignored, the milestone limit raised on the screen alone — and each had to make the right check fail by name. Three checks earlier in this same body of work were found to be passing without testing anything, and in every case it was running them against known-broken code that exposed it, never reading them.
...
`LICENSE` remains a byte-for-byte copy of the canonical file in the SPERT® Suite landing-page repository, differing only in the project repository URL on line 4. It goes from 726 lines to 756. What the licence permits is unchanged; what changed is the set of conditions attached to it, which now number six rather than four.

### Added
- **The author's name may not be used to endorse or promote a product built from this software** without permission (GPL v3 §7(d)). Nothing else in the licence covered this: the project's trademarks are protected whether the licence mentions them or not, but a personal name has no such protection, and another condition requires that name to stay in the source code.
- **Anyone who resells this software with a warranty or support contract of their own covers any liability those promises impose on the original author** (§7(f)). The standard licence already permits a reseller to make such promises; this makes clear they are theirs to stand behind.

### Changed
- **The condition covering on-screen credit was rewritten.** It used to require any modified version with a user interface to *display* a notice. §7(b) authorises requiring that existing notices be *preserved*, not that new ones be created, and §5(d) says outright that a modified work need not add such notices where the original had none. A condition reaching past what §7 allows can simply be deleted by whoever receives the software — so the old wording protected nothing. It now requires that where a modified version already shows Appropriate Legal Notices, the original author's name is kept among them.
- **A modified version may no longer misrepresent where this software came from**, claiming §7(c)'s previously unused first half.
- **The trademark condition gains a narrow carve-out**: naming this project in order to describe honestly what a fork was derived from is not itself prohibited, provided it does not suggest this project endorses the result. This resolves the tension where the trademark condition granted no rights to the marks while the marking condition required a fork to distinguish itself from them.
- **Every condition now tracks the wording of the §7 subsection that authorises it**, and the registration recital is date-stamped, since asserting live USPTO registration as bare present fact acquires a shelf life.
...
**The connection is read-only, and that is enforced in three independent places rather than promised once.** There is no tool an assistant could call to change anything in Forecaster — the server does not define one. The pairing is created with writes refused, permanently: the flag is fixed at creation and excluded from the fields the browser is allowed to update afterwards. And the server re-checks that flag inside the transaction that would append a change, before any write occurs. An assistant that tries anyway gets a refusal that tells it the condition is permanent and not to retry.

**Read Mode is the only consent toggle, and turning it off actually removes the data.** With Read Mode on, a copy of the currently open project is uploaded to SPERT's cloud so the assistant can read it — and this happens whether your storage setting is Local or Cloud, which the consent dialog says plainly. Turning Read Mode off deletes that copy immediately and keeps the pairing alive; disconnecting or signing out deletes both. Only the project you have open is ever uploaded, never your whole workspace, and the browser refuses to write a snapshot at all while Read Mode is off — which matters because that upload is the one place a stale write could otherwise re-create data you had just withdrawn.

**The snapshot tells the assistant what it is not looking at.** This turned out to be the hard part. An assistant handed a forecast will confidently quote it — including when the numbers are stale, when they come from a distribution you have hidden in Settings, or when they belong to a milestone that is not the one on your screen. So every snapshot carries a freshness status (`fresh`, `stale` with the specific reason, `recomputing`, or `absent`), the set of distributions actually visible to you as distinct from the set the app computed, a flag on each scope saying whether it is the one you are reading, and an explicit list of what the snapshot does not carry — the raw trial data, the charts, other projects, the cumulative-probability curve, and the large percentage in the forecast headline, which is a different calculation from the percentile you selected and reads higher. Two further tools, needing no pairing at all, explain the method and define the vocabulary; both exist because assistants reliably describe SPERT as three-point PERT with the `(O + 4M + P) / 6` weighted average, which it is not and never has been.

Requires cloud configuration, so the section does not appear in a local-only build. The production build, ESLint (`--max-warnings=0`), and all 1,163 tests (51 new) pass. The server half ships as SPERT MCP `1.12.0` (48 → 51 tools).

### Added

- **Connect AI section in Settings** — a consent dialog, a single-use pairing code that refreshes while idle, a Read Mode toggle, a live "assistant connected" indicator, and Disconnect. Copy buttons for both the bare code and a prepared prompt that tells your assistant how to read the snapshot correctly.
- **Three read-only tools on the SPERT MCP server**: one that reads the paired project, and two ungated explainers covering the forecasting method and the app's vocabulary.

### Internal

- **The snapshot is built by an explicit allowlist — every field added by name, with no object spread anywhere in the module.** The stores it reads from carry workspace-reconciliation tokens and export-attribution fields that must never leave the browser, and an allowlist cannot leak a field nobody wrote. Four tests assert this against a fixture where every such field holds a distinctive sentinel: no key beginning with an underscore, no `exportName`/`exportId` key, and no sentinel *value* anywhere in the serialized body — the last of which catches a leak that renamed its key on the way out. All four assertions were confirmed to fail against a deliberately introduced spread, and they run against the post-sanitize artifact, which is what actually reaches the cloud.
- **Every freshness-ladder row is unit tested with its user-facing reason.** Including the two that are easy to miss: a backlog of `"0"` is reported as *blocked*, not merely un-run, because the run prerequisites test the raw string (where `"0"` is truthy) while the handler bails on the parsed value — left unhandled, an assistant would advise running a forecast that silently does nothing forever. And the "ask the user to recompute" closing sentence becomes "the forecast cannot be re-run until…" when a re-run is currently impossible, which is reachable by running a forecast and then clearing the backlog field.
- **What the three summary dropdowns report is the value they RENDER, not the value they store.** Two of the three render a derived fallback: the distribution picker falls back to the first enabled option when your stored choice is no longer offered, and the scope picker falls back to "Entire Project" when the selected milestone completes. Emitting the stored cells would have put `lognormal` on the wire while the screen read "Gamma". The scope list is filtered on completion alone — deliberately not the two-filter rule the charts use — so a milestone hidden from charts remains selectable, exactly as the app behaves.
- **Snapshot size degrades rather than skipping the write.** A skipped write leaves the previous snapshot in place and stale, so an assistant reads outdated numbers with no signal at all. Over budget, the builder first drops percentile grids for distributions you cannot see, then falls back to configuration only — both marked `bodyDegraded` and disclosed. The budget is an injectable parameter so both steps are unit tested; a typical project is an order of magnitude inside it.
- **The session document is asserted to carry exactly ten keys**, because the Firestore create rule permits exactly those and rejects any extra — a mistake that would surface only in production, at pairing time, as an opaque permission error. The likeliest extra key is the one the client *reads* to drive the connected indicator but must not write.
- The heartbeat reads the open project from a ref rather than a closure, so a timer scheduled before a project switch cannot pin the previous project onto the session; it stops itself on a deleted or forbidden session rather than failing every 30 seconds for the life of the tab; and a `visibilitychange` listener re-announces the browser on tab focus, since background-tab throttling can otherwise push a returning tab outside the server's connected window.
- Snapshot writes are debounced two seconds, keyed on a content digest rather than object identity (cloud delivery returns a fresh object literal every time), with the debounce bypassed on a project switch and on a simulation starting — without the latter, the "recomputing" status would never be published at all, since clicking Run changes nothing else the digest sees.

## v0.36.0 - 2026-07-25

Internal — forecast results and the Forecast tab's view state moved out of component-local `useState` and into a dedicated, non-persisted store. **Nothing on screen changes.** The motivation is a defect that has been present since the Forecast tab existed: `AppShell` renders tabs conditionally, so leaving the Forecast tab unmounted the whole subtree — destroying every simulation result and terminating the Monte Carlo worker. Coming back meant running the forecast again, which with an unseeded RNG produced *different dates for the same inputs*. Target dates, custom percentile selections, the scope and distribution pickers, and the scope-growth toggles all reset the same way. They now live in a `forecast-results` store keyed per project, so a tab switch is a no-op: the percentile tables, the headline sentence, the milestone breakdown, and the burn-up series come back byte-identical because the record retains the worker's own result objects rather than recomputing from the raw trial arrays.

The second half is that the app now records **which inputs produced the results currently on screen**. Each run publishes a `runConfig` — backlog, velocity mean and SD, forecast start date, trial count, cadence, mode, scope growth, plus digests of the included sprint velocities, the milestone thresholds, and the enabled productivity adjustments — captured at *run start*, before the worker is dispatched, so it describes what the simulation actually consumed rather than whatever the form holds when the run finishes. That makes "are these results still current?" a decidable question for the first time, and it is what lets auto-recalculate stop re-running a forecast whose inputs have not moved.

The one rule that matters here, and the reason it is stated three times in the source: **the same builder produces both the record's `runConfig` and the value freshness is checked against.** If those two code paths ever diverge on a single field, every record is stale the instant it is written, auto-recalculate fires again, and — because auto-recalculate is on by default — the app runs the worker in a continuous loop. A concrete instance of that divergence already existed in the code this release replaces: the scope-growth resolver returns `undefined` while the run config wants `null`, and `undefined !== null` under field comparison, so the most common configuration of all (scope growth off) would have been permanently stale. The normalization now happens in exactly one place, and a test mutating it back to the broken form turns the suite red rather than hanging.

Three deliberate behavior changes came with the per-project keying, all reviewed rather than discovered: the **Model scope growth** toggle is now remembered per project instead of surviving a project switch; the two custom-percentile sliders and the results-table percentile pills no longer re-seed from Settings on every return to the Forecast tab (they keep the choice made for that project, which is the point of keying them per project); and the two custom-percentile result columns now show empty rather than the previous project's numbers during the frame in which a project switch is still settling. The production build, ESLint (`--max-warnings=0`), and all 1,112 tests (42 new) pass; the sample project was verified end-to-end in a browser, including that a forecast auto-runs exactly once and that a tab switch and return reproduces the same dates.

### Changed

- **Forecast results survive leaving the Forecast tab.** Results, per-milestone slices, the raw trial arrays, and the in-flight simulation flag moved from `useForecastState`'s `useState` cells into `src/shared/state/forecast-results-store.ts`, created deliberately **without** `persist` — the record holds six raw sorted trial arrays per scope, which must never be serialized, and results describe a run that happened in this session. Returning to the tab now renders the retained record instead of starting a new simulation.
- **Forecast view state is remembered per project.** The target date, both custom percentiles, the results-table percentile selection, the milestone selector, the scope-growth toggles, and the summary panel's scope / distribution / percentile pickers are keyed by project id. A project-change reset effect used to clear some of these — the comment explaining why ("a target date from project A shouldn't bleed into project B") now travels with the store field, so the mechanism is per-project keying rather than a reset.
- **Auto-recalculate no longer re-runs a forecast whose inputs are unchanged.** The effect now checks the resolved project's record against a freshly built input snapshot and returns early when nothing that affects the numbers has moved. Editing a milestone's backlog, toggling a sprint's inclusion, or changing an enabled productivity adjustment still triggers exactly one re-run.

### Internal

- **`buildForecastInputSnapshot` is the single producer of both the published `runConfig` and the staleness comparand**, and it is called once per run, before the `await`, alongside the captured project id and generation token. The `resolveScopeGrowthPerSprint(...) ?? null` normalization lives inside it and nowhere else. The worker payload is now read off the same snapshot, so a record provably describes the numbers the simulation consumed.
- **Both forecast hooks' derivations were extracted whole** into `shared/lib/forecast-derivations.ts` (`deriveSprintData`, `deriveForecastInputs`, and the helpers they need), with `useSprintData` and `useForecastInputs` becoming `useMemo` wrappers over them. The freshness check and the snapshot builder run outside React and cannot call a hook, so extracting only one hook's half would have left both unable to reach resolved sprint dates, velocity stats, scope-change stats, and the completed-sprint count. `statistics.ts`, `simulation-generation.ts`, the Monte Carlo result types, the scope sentinel, and the four constants those derivations depend on moved to `shared/` accordingly; the feature's former paths re-export every name, so no existing import site changed.
- **A publish is suppressed entirely if the resolved project changed mid-run.** The viewed-project selector falls back to `projects[0]`, so a collaborator deleting the open project while a simulation is in flight could otherwise stamp one project's results with another's id — and the read gate would then match and show them under the wrong name. The run captures the resolved id at start and re-resolves before publishing.
- **Every path that stops a run clears the in-flight flag under a run-token check**: worker unmount (a store write, since React discards state updates on an unmounting component), the success and error handlers, the rejection path, and the branch where the worker ref is null — which also has to reject and clear the pending ref, since it sits inside the promise executor and clearing alone would leave the promise permanently unsettled. Without the token check, an aborted run's late callback would clear the flag while its replacement was still running.
- **The record and the per-project view-state map are purged at the same five sites** — sign-out, both cloud→local switches, full import, merge-import for replaced project slots, and project deletion. The map holds target dates and percentile selections, the same shape of residue `forecastInputs` and the change log are already cleared for on a shared device. Neither is cleared on unmount or on a cloud snapshot delivery.
- 42 new tests. The highest-value one publishes through `handleRunForecast` itself and builds the comparand independently afterwards, because the obvious version — build a snapshot, publish from it, compare against the same snapshot — is a tautology that passes under every implementation including the broken one. A loop tripwire asserts the worker is invoked exactly once with auto-recalculate on, and its worker mock carries a circuit breaker: a genuine divergence starves the event loop, so without it the suite would hang instead of reporting. Both were confirmed to fail against a deliberately reintroduced divergence.
...
Cloud storage hardening — seven correctness and data-integrity fixes identified by a systematic audit of v0.34.0 against the SPERT Suite cloud storage pattern catalog. Before v0.35.0, a user who cleared a project's optional date fields saw the old dates silently reappear after a browser refresh; sign-out cleanup ran after Firebase credentials were already revoked rather than before; an in-flight Monte Carlo simulation could write results into a store that sign-out had already cleared; and the data-loss guard that protected un-migrated local projects was permanently blocking access-revocation notifications from propagating. Three additional issues: the Settings tab's custom-percentile inputs snapped to their minimum value mid-keystroke, a 500 ms save debounce left click-driven changes vulnerable to silent loss on fast browser refresh, and mobile Safari and bfcache navigations were excluded from the save-flush path.
...
Import retrograde — six refinements identified by a full audit of the Robust Import guide's 90 pitfalls. Before v0.34.0, the import flow had four user-observable rough edges and two latent correctness gaps that only surfaced under specific cloud-mode + collaborator conditions. After v0.34.0, the "Importing..." label actually paints during the synchronous merge work, the Import button is disabled while cloud projects are still hydrating from Firestore (closing a race where conflict detection ran against stale local data), the copy-suffix on import matches the Clone button convention rather than the legacy "(2)" suffix that was inconsistent with the rest of the app, and a name-conflict replace in cloud mode no longer destroys prior collaborator sharing when the importer owns the existing project. The user-visible behavior change (copy-suffix rename) is documented as a deliberate deviation from the import spec in [docs/SPEC_DEVIATIONS.md](docs/SPEC_DEVIATIONS.md).
...
Inline rename for milestones on the Forecast tab. Clicking a milestone name on the Milestones panel now swaps the cell to a text input in place, so the common "I just want to fix this typo" workflow no longer requires opening the full Pencil-button form (which also asks for `Remaining Work` and `Color` the user has no reason to touch). The full edit form remains the path for everything else — this is a power-user shortcut layered on top, not a replacement.
