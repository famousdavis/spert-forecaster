...
**Reusable CRUD pattern**: `CollapsibleCrudPanel<T>` provides a generic expand/collapse panel with add/edit/delete state machine, used by Milestones and Productivity Adjustments. The list always remains visible while adding or editing — the form renders below the list to preserve context. Name fields auto-focus on form open. `ListRowActions` provides shared Edit/Delete button markup. Both MilestoneList and ProjectList support HTML5 drag-and-drop reordering, but no longer by the same mechanism. ProjectList still tracks a hovered *row* (`draggedIndex`/`dragOverIndex`, splice at the row index) and marks it with a box. MilestoneList tracks a *gap* between rows (`draggedIndex`/`dropGap`, `forecast/lib/drag-reorder.ts`), resolved from the pointer's position within the hovered row, because it marks its target with an insertion line — and a line has to name a gap, which a row index cannot. The row-index form was the v0.40.1 bug: it resolved the ambiguity one way when dragging down and the other way when dragging up, while always drawing the line below.
